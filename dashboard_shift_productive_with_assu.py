...
# ---------------------------
# Load and Process Data Functions - MEMORY OPTIMIZED
# ---------------------------

@st.cache_data(ttl=3600, max_entries=1)
def load_data():
    """
    Load parquet file directly from Hugging Face using DuckDB.
    Memory-efficient: loads only necessary columns and filters early.
    """
    ...
    with st.spinner('Loading data from Hugging Face...'):
        try:
            # Use DuckDB to read parquet directly from URL - very memory efficient
            ...
            raise FileNotFoundError(f"Could not load data from URL. Please check if the file is publicly accessible.")
...
def get_shift_analysis(miner_id=None):
    """Process data with memory-efficient DuckDB queries"""
    df = load_data()
    ...
    # DuckDB query - processes data efficiently without loading everything into memory
    query = f"""
    WITH base AS (
        SELECT
            miner_id,
...
def show_dashboard_page():
    ...
    try:
        ...
        month_options = result_df.groupby(['sort_key', 'month_year'], observed=True).size().reset_index()[['sort_key', 'month_year']]
        ...
        with tab2:
            ...
            if show_detail:
                ...
                if len(filtered_df) > 10000:
                    st.warning("⚠️ Showing first 10,000 records only to prevent memory issues")
